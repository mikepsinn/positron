...
The GitHub API uses OAuth 2.0 for authentication. This means that the agent must be authorized to access the repository before it can make any requests. The agent uses a personal access token to authenticate with GitHub. To generate a personal access token, refer to the relevant section in the [GitHub documentation](https://docs.github.com/en/rest/overview/other-authentication-methods). This token is stored in the agent's configuration file, in the format and location specified in the configuration file located at /path/to/configuration.yaml.

### Endpoints

The GitHub API provides endpoints for creating and managing issues, pull requests, and other aspects of the repository. The agent uses these endpoints to create and manage documentation. The agent uses these endpoints to create and manage documentation.
